...
def generate_docx(data: dict) -> bytes:
    ...
    for domain, meta in FLOWER_DOMAINS.items():
        d_score = domain_scores.get(domain, 0)
        _heading(doc, f"Pétalo {meta['petal_num']}: {meta['icon']} {domain}  [{d_score}/5]",
                 2, C_MED, 11)
        _para(doc, meta.get("desc",""), italic=True, color=C_GRAY, size=9)

        ind_rows = []
        for ind in meta["indicators"]:
            key  = ind["key"]
            if data.get(f"{key}_na"): continue
            val  = data.get(key)
            if val is not None:
                sl = SCORE_SCALE.get(int(val), {}).get("label", str(val))
                ind_rows.append((ind["label"], f"{val}/5 — {sl}"))
        if ind_rows: _kv_table(doc, ind_rows)

        for pk, plabel in [
            (f"flower_practicas_actuales_{domain}", "✅ Prácticas actuales"),
            (f"flower_practicas_futuras_{domain}",  "🚀 Desarrollo futuro"),
            (f"flower_notas_{domain}",               "📝 Notas"),
        ]:
            v = data.get(pk, "")
            if v:
                p = doc.add_paragraph()
                ...
                rb = p.add_run(f"{plabel}: ")
                rb.bold = True; rb.font.size = Pt(9); rb.font.color.rgb = C_DARK
                rv = p.add_run(str(v))
                rv.font.size = Pt(9); rv.font.color.rgb = C_GRAY; rv.italic = True
        doc.add_paragraph()
