...
# ── Datos de pétalos embebidos directamente (sin caché, sin archivos) ────────
...
def _get_petalos():
    """Devuelve la lista de pétalos. Sin caché para evitar datos obsoletos."""
    if isinstance(_PETALOS_DATA, list):
        return _PETALOS_DATA
    if isinstance(_PETALOS_DATA, str):
        return json.loads(_PETALOS_DATA)
    return []
