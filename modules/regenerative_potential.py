...
def render():
    ...
    # ── Info IPR ──────────────────────────────────────────────────────────
    with st.expander("ℹ️ ¿Qué es el IPR? · ¿De dónde vienen los 7 pétalos?", expanded=False):
        st.markdown("""
**Origen de los 7 pétalos de la Flor de la Permacultura**

Los 7 pétalos son los dominios de transformación definidos por David Holmgren en su obra
*Permacultura: Principios y senderos más allá de la sustentabilidad* (2002). Holmgren los
concibió como los ámbitos clave que deben transformarse para transitar hacia una cultura
sostenible durante el descenso energético. Cada pétalo parte del núcleo ético y de los
principios de diseño, y se expande hacia la organización social, económica y territorial.

La Flor de la Permacultura no es un marco inventado por LivLin — es una referencia
internacional utilizada en miles de proyectos en más de 60 países. LivLin la adapta
como instrumento de diagnóstico, añadiendo un catálogo de prácticas concretas para
cada pétalo, especialmente en contextos urbanos y periurbanos.

📚 Fuente: Holmgren, D. (2002). *Permacultura: Principios y senderos más allá de la
sustentabilidad*. Holmgren Design Services. https://permacultureprinciples.com/es/
        """)
    ...
    def _petal_done(i):
        ...
        others_new = data.get(f"petalo_{i}_otros_new", [])
        return (any(v for v in obs.values()) or
                any(v for v in new.values()) or
                bool(otros_obs) or bool(others_new))

    # ── Navigation: Results tab FIRST, then 7 petals ────────────────────
    st.markdown(
        '<div style="background:#F0FFF4;border:1.5px solid #52B788;border-radius:8px;'
        'padding:0.5rem 1rem;margin-bottom:0.5rem;font-size:0.8rem;color:#1B4332;">'
        '💡 <strong>Consejo:</strong> Comienza en 📊 Resumen para ver el estado global, '
        'luego completa cada pétalo. Los pétalos respondidos muestran ✅.</div>',
        unsafe_allow_html=True)
    ...
    for i, p in enumerate(petalos):
        ...
        badge = "✅" if done else "○"
        done_badges.append(badge)

    tab_labels = ["📊 Resumen IPR"]
    tab_labels += [f"{done_badges[i]} {PETAL_ICONS[i]} {p['nombre'][:18]}"
                   for i, p in enumerate(petalos)]
