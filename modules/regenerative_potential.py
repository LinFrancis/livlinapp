...
def render():
    ...
    try:
        petalos = _load_petalos_v13()
        if not petalos or not isinstance(petalos, list) or not isinstance(petalos[0], dict):
            st.error(f"❌ Error cargando pétalos: formato inesperado — {type(petalos)}, primer elemento: {petalos[0] if petalos else 'vacío'}")
            return
    except Exception as _e:
        st.error(f"❌ Error cargando base de prácticas: {_e}")
        return
